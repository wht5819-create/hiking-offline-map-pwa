...
function drawRoute(width, height) {
  ...
  for (const [index, segment] of state.routeSegments.entries()) {
    ...
    const color = index % 2 === 0 ? '#f04438' : '#39d353';
    drawRouteStroke(segmentPoints, color, 5);
  }
...
function drawWaypointPins(width, height) {
  for (const waypoint of state.waypoints.slice(0, 30)) {
    ...
    drawPinLabel(point, '#d92828', waypoint.name, Number.isFinite(waypoint.ele) ? `${Math.round(waypoint.ele)}m` : '');
  }
}
...
function updateMapPackageSummary() {
  ...
  if (!item) {
    ...
    elements.mapStatus.textContent = '魯地圖 Mapsforge .map 已登記；目前可見底圖使用線上地形圖，離線請匯入 `.mbtiles`。';
